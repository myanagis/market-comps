...
if prompt or st.session_state.get("manual_proceed", False):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            if not operations and "action" in action_data:
                operations = [action_data]
            ...
            for op in operations:
                ...
                if op_action in ["extract", "update_market_map"]:
                    if op_action == "extract":
                        comps = op.get("extracted_companies", [])
                    else:
                        comps = op.get("market_map_update", {}).get("companies", [])
                    
                    for comp in comps:
                        if isinstance(comp, str): continue
                        name = comp.get("name")
            ...
            for op in operations:
                ...
                action_data = op  # Rebind action_data for the inner loop block
