...
docs = db.query(SourceDocument).join(PipelineRun).outerjoin(Pipeline).order_by(SourceDocument.created_at.desc()).all()
if not docs:
    st.warning("No imported documents found.")
    st.stop()

doc_opts = {d.id: f"{d.source_url} (Ingestion Run #{d.pipeline_run_id}) [Created: {d.created_at.strftime('%Y-%m-%d %H:%M')}]" for d in docs}
...
if selected_doc_id:
    ...
    with tab4:
        ...
        muts = db.query(AuditTrail).join(ExtractionJob).filter(ExtractionJob.pipeline_run_id == run.id).all()
        if muts:
            df = pd.DataFrame([{
