...
def build_competition_tables(result: CompetitionResult):
    """Return (public_df, private_df) for display (no desc/diff/sources in table)."""
    pub_rows, priv_rows = [], []
    for c in result.competitors:
        if c.type == "public":
            ...
            priv_rows.append({
                ...
                "Year": fmt_int(c.funding_year),
            })
    return pd.DataFrame(pub_rows), pd.DataFrame(priv_rows)
...
def generate_html_report(
    ...
    if comp_result:
        ...
        pub_df, priv_df = build_competition_tables(comp_result)
        # detail rows
        details = ""
        for c in comp_result.competitors:
            sources = " ".join(f'<a href="{u}">{_domain(u)}</a>' for u in c.source_urls)
            ...
            if c.type == "public":
                ...
                fin = f"Latest Round: {c.latest_round or '—'} &nbsp;|&nbsp; Raised: {fmt_currency(c.amount_raised_usd)} &nbsp;|&nbsp; Year: {fmt_int(c.funding_year)}"
            details += f"""
            <div class="card">
        ...
        <p>{landscape}</p>
        <h3>Public Competitors</h3>{tbl(pub_df)}
        <h3>Private Competitors</h3>{tbl(priv_df)}
...
with col_q:
    query = st.text_input(
        ...
        placeholder="Company name or industry (e.g. 'Stripe', 'cloud ERP', 'digital payments')",
        key="query_input",
    )
with col_d:
    description = st.text_input(
        ...
        label_visibility="collapsed",
        placeholder="Brief description — helps the competition search (e.g. 'B2B fintech payments API')",
...
if run_clicked and query.strip() and chorus_models:
    ...
    with prog.container():
        st.info(f"⏳ **Phase 1/2** — Querying {len(chorus_models)} models for competitors of **{query}**…")
    try:
        with st.spinner(""):
            finder = CompetitionFinder(chorus_models=chorus_models)
            comp_result = finder.run(company=query.strip(), description=description.strip())
    ...
    with prog.container():
        st.info(f"⏳ **Phase 2/2** — Querying {len(chorus_models)} models for public comparable tickers…")
    try:
        with st.spinner(""):
            engine = ChorusCompsEngine(chorus_models=chorus_models)
